...
const search_country_element = document.querySelector(".searchcountry");
const country_list_element = document.querySelector(".countries");
const chang_country_btn = document.querySelector(".changeValues");
const close_list_btn = document.querySelector("#close");
const input = document.getElementById('search-input')

// CREATE TE COUNTRY LIST
let number_countries = country_list.length;
function createCountryList(){
    let num_countries = country_list.length;
    let id = 0;
    country_list.forEach( (country, index) => {
        if(index % Math.ceil(num_countries/ul_list)==0){
            ul_list_id = `list-${id}`;
...
let ul_list = 3;
createCountryList();
chang_country_btn.addEventListener("click",clicked);
 function clicked() {
     input.value="";
     resetcountriesList();
 ...
 close_list_btn.addEventListener("click",closy);
 function closy() {
    search_country_element.classList.toggle("hide");
 }
